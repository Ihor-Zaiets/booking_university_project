...
public class ReservationRepositoryTest {
    ...
    public void shouldReturnTrueOnReservationOverLay() {
        ...
        startDateEqualsEndDate.setEndDate(
                Timestamp.valueOf(existingReservation.getEndDate().toLocalDateTime().plusDays(1))
        );
        startDateBetweenStartDateAndEndDate.setStartDate(
                Timestamp.valueOf(existingReservation.getEndDate().toLocalDateTime().minusSeconds(1))
        );
        startDateBetweenStartDateAndEndDate.setEndDate(
                Timestamp.valueOf(existingReservation.getEndDate().toLocalDateTime().plusDays(1))
        );
        endDateBetweenStartDateAndEndDate.setEndDate(
                Timestamp.valueOf(existingReservation.getEndDate().toLocalDateTime().minusSeconds(1))
        );
        endDateBetweenStartDateAndEndDate.setStartDate(
                Timestamp.valueOf(existingReservation.getStartDate().toLocalDateTime().minusDays(1))
        );
        endDatesEqualsStartDate.setEndDate(existingReservation.getStartDate());
        endDatesEqualsStartDate.setStartDate(
                Timestamp.valueOf(existingReservation.getStartDate().toLocalDateTime().minusDays(1))
        );

        assertTrue(
                reservationRepository.isReservationExistsForApartmentInDates(
                        startDateEqualsEndDate.getApartment().getId(),
                        startDateEqualsEndDate.getStartDate(),
                        startDateEqualsEndDate.getEndDate()
                )
        );
        assertTrue(
                reservationRepository.isReservationExistsForApartmentInDates(
                        startDateBetweenStartDateAndEndDate.getApartment().getId(),
                        startDateBetweenStartDateAndEndDate.getStartDate(),
                        startDateBetweenStartDateAndEndDate.getEndDate()
                )
        );
        assertTrue(
                reservationRepository.isReservationExistsForApartmentInDates(
                        endDateBetweenStartDateAndEndDate.getApartment().getId(),
                        endDateBetweenStartDateAndEndDate.getStartDate(),
                        endDateBetweenStartDateAndEndDate.getEndDate()
                )
        );
        assertTrue(
                reservationRepository.isReservationExistsForApartmentInDates(
                        endDatesEqualsStartDate.getApartment().getId(),
                        endDatesEqualsStartDate.getStartDate(),
                        endDatesEqualsStartDate.getEndDate()
                )
        );
        assertTrue(
                reservationRepository.isReservationExistsForApartmentInDates(
                        datesTheSame.getApartment().getId(),
                        datesTheSame.getStartDate(),
                        datesTheSame.getEndDate()
                )
        );
    }
    ...
    public void shouldReturnFalseIfNoReservationOverlayCaused() {
        ...
        reservationBefore.setStartDate(
                Timestamp.valueOf(reservationBefore.getStartDate().toLocalDateTime().minusDays(8))
        );
